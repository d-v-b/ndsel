# ndsq Contract + Rust Reference — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Produce the ndsq contract (normative spec, JSON Schema, conformance corpus) plus a complete, corpus-verified Rust reference implementation of `normalize`.
...
**Tech Stack:** Rust (edition 2024), `serde` + `serde_json` (de/serialization), `jsonschema` (schema validation in the corpus runner). Reference design: `docs/superpowers/specs/2026-06-13-ndsq-spec-design.md`.
...
```
spec/ndsq.md                         normative spec (prose, derived from the design doc)
schema/ndsq.schema.json              JSON Schema (draft 2020-12), syntax only
conformance/*.json                   fixtures: { input, normalized } or { input, error }
conformance/README.md                fixture format contract
rust/ndsq/Cargo.toml                 lib crate + deps
rust/ndsq/src/lib.rs                 public API: Message, normalize(); module wiring
rust/ndsq/src/error.rs               NdsqError + reason codes
rust/ndsq/src/value.rs               IndexValue (i64 / ±inf) + ImplicitValue (bracket convention)
rust/ndsq/src/domain.rs             Domain (rank, inclusive_min, exclusive_max, labels) + upper-bound normalization
rust/ndsq/src/output.rs              OutputMap (constant / single_input_dimension / index_array)
rust/ndsq/src/transform.rs          Transform struct + canonicalization
rust/ndsq/src/shorthand.rs          desugar point/box/slice/points -> Transform
rust/ndsq/tests/conformance.rs      corpus runner (schema-validate + normalize + compare)
```
...
Create `spec/ndsq.md` as the clean, normative version of the design doc. Copy the substance of these sections from `docs/superpowers/specs/2026-06-13-ndsq-spec-design.md`, rewritten as a specification (imperative "MUST/MAY", no "decisions log" or "open items meta"):
...
```bash
git add spec/ndsq.md
git commit -m "docs: add normative ndsq spec prose"
```
...
**Files:**
- Modify: `rust/ndsq/Cargo.toml`
- Create: `rust/ndsq/src/lib.rs`
- Delete: `rust/ndsq/src/main.rs`
- Create (empty module stubs): `rust/ndsq/src/{error,value,domain,output,transform,shorthand}.rs`

- [ ] **Step 1: Convert to a library crate with dependencies**

Replace `rust/ndsq/Cargo.toml` with:
...
Delete `rust/ndsq/src/main.rs`. Create `rust/ndsq/src/lib.rs` with ONLY the module declarations. The public API (`Message`, `normalize`, `parse`, and re-exports) is added in Task 9, once every module exists — so Tasks 3–8 each compile and test independently:
...
Run: `cargo build -p ndsq 2>&1 | tail -5`
Expected: clean build.

- [ ] **Step 4: Commit**

```bash
git add rust/ndsq/Cargo.toml rust/ndsq/src/
git rm rust/ndsq/src/main.rs
git commit -m "chore(rust): convert ndsq to a library crate with serde"
```
...
**Files:**
- Create: `rust/ndsq/src/error.rs`
- Test: inline `#[cfg(test)]` in `error.rs`

- [ ] **Step 1: Write the failing test**

Put this at the bottom of `rust/ndsq/src/error.rs`:
...
Run: `cargo test -p ndsq --lib error 2>&1 | tail -20`
Expected: FAIL — `Reason` not found / does not compile.

- [ ] **Step 3: Write the implementation**

Put this ABOVE the test module in `rust/ndsq/src/error.rs`:
...
```bash
git add rust/ndsq/src/error.rs
...
**Files:**
- Create/replace: `rust/ndsq/src/value.rs` (IndexValue portion)
- Test: inline in `value.rs`

- [ ] **Step 1: Write the failing test**

Add to `rust/ndsq/src/value.rs`:
...
Run: `cargo test -p ndsq --lib index_value 2>&1 | tail -20`
Expected: FAIL — `IndexValue` not defined.

- [ ] **Step 3: Write the implementation**

Add ABOVE the test module in `rust/ndsq/src/value.rs`:
...
```bash
git add rust/ndsq/src/value.rs
...
**Files:**
- Modify: `rust/ndsq/src/value.rs`
- Test: inline in `value.rs`

- [ ] **Step 1: Write the failing test**

Add a second test module to `rust/ndsq/src/value.rs`:
...
Run: `cargo test -p ndsq --lib implicit_value 2>&1 | tail -20`
Expected: FAIL — `ImplicitValue` not defined.

- [ ] **Step 3: Write the implementation**

Add to `rust/ndsq/src/value.rs` (above the implicit test module):
...
**Files:**
- Create: `rust/ndsq/src/domain.rs`
...
Create `rust/ndsq/src/domain.rs` with this test at the bottom:
...
Run: `cargo test -p ndsq --lib domain 2>&1 | tail -20`
Expected: FAIL — `RawDomain` / `Domain` not defined.

- [ ] **Step 3: Write the implementation**

Add ABOVE the test module in `rust/ndsq/src/domain.rs`:
...
```bash
git add rust/ndsq/src/domain.rs
...
**Files:**
- Create: `rust/ndsq/src/output.rs`
...
Create `rust/ndsq/src/output.rs` with this test at the bottom:
...
Run: `cargo test -p ndsq --lib output 2>&1 | tail -20`
Expected: FAIL — types not defined.

- [ ] **Step 3: Write the implementation**

Add ABOVE the test module in `rust/ndsq/src/output.rs`:
...
```bash
git add rust/ndsq/src/output.rs
...
**Files:**
- Create: `rust/ndsq/src/transform.rs`
...
Create `rust/ndsq/src/transform.rs` with this test at the bottom:
...
Run: `cargo test -p ndsq --lib transform 2>&1 | tail -20`
Expected: FAIL — `Transform` not defined.

- [ ] **Step 3: Write the implementation**

Add ABOVE the test module in `rust/ndsq/src/transform.rs`:
...
Run: `cargo test -p ndsq --lib 2>&1 | tail -20`
...
```bash
git add rust/ndsq/src/transform.rs
...
**Files:**
- Create: `rust/ndsq/src/shorthand.rs`
- Modify: `rust/ndsq/src/lib.rs` (add the public API: `Message`, `normalize`, `parse`, re-exports)
- Test: inline in `shorthand.rs`

- [ ] **Step 1: Write the failing test**

Create `rust/ndsq/src/shorthand.rs` with this test at the bottom:
...
Run: `cargo test -p ndsq --lib point_box 2>&1 | tail -20`
Expected: FAIL — `Point` / `BoxSel` not defined.

- [ ] **Step 3: Write the implementation**

Add ABOVE the test module in `rust/ndsq/src/shorthand.rs`:
...
Now add the public API to `rust/ndsq/src/lib.rs` (append below the `mod` declarations — every referenced type now exists):
...
```bash
git add rust/ndsq/src/shorthand.rs rust/ndsq/src/lib.rs
...
**Files:**
- Modify: `rust/ndsq/src/shorthand.rs`
- Test: inline in `shorthand.rs`
...
Add a test module to `rust/ndsq/src/shorthand.rs`:
...
Run: `cargo test -p ndsq --lib slice_tests 2>&1 | tail -20`
Expected: FAIL — `unimplemented!` panics / assertions fail.

- [ ] **Step 3: Write the implementation**

Replace `Slice::desugar` in `rust/ndsq/src/shorthand.rs` with:
...
```bash
git add rust/ndsq/src/shorthand.rs
...
Run: `cargo test -p ndsq --lib points_tests 2>&1 | tail -20`
Expected: FAIL — `unimplemented!` panics.

- [ ] **Step 3: Write the implementation**

Replace `Points::desugar` in `rust/ndsq/src/shorthand.rs` with:
...
**Files:**
- Modify: `rust/ndsq/src/lib.rs` (add tests only)
...
Add to the bottom of `rust/ndsq/src/lib.rs`:
...
Run: `cargo test -p ndsq --lib tests 2>&1 | tail -20`
...
**Files:**
- Create: `schema/ndsq.schema.json`
...
```json
{
  ...
  "$id": "https://ndsq.dev/schema/ndsq.schema.json",
  "title": "ndsq message",
  "$defs": {
    "indexValue": {
      "oneOf": [
...
```bash
git add schema/ndsq.schema.json
git commit -m "feat: JSON Schema for ndsq messages (syntax validation)"
```
...
- Create: `rust/ndsq/tests/conformance.rs`
...
```rust
//! Runs every fixture in ../../conformance against `ndsq::normalize`,
...
fn repo_root() -> PathBuf {
    // tests run with CWD = rust/ndsq; corpus + schema are two levels up.
    Path::new(env!("CARGO_MANIFEST_DIR")).join("../..").canonicalize().unwrap()
}

fn load_schema() -> jsonschema::Validator {
    let path = repo_root().join("schema/ndsq.schema.json");
...
fn corpus_round_trips() {
    ...
    for file in corpus_files() {
        ...
        for case in cases {
            ...
            let msg = ndsq::parse(&input.to_string());
            ...
            if let Some(expected_reason) = case.get("error").and_then(|e| e.as_str()) {
                ...
                let err = match msg {
                    Ok(m) => ndsq::normalize(m).expect_err(&format!("{name}: expected error")),
                    Err(e) => e,
                };
                ...
                let normalized = ndsq::normalize(msg.expect(name)).expect(name);
...
Run: `cargo test -p ndsq --test conformance 2>&1 | tail -30`
...
```bash
git add conformance/ rust/ndsq/tests/conformance.rs
...
Replace `README.md` with an overview: what ndsq is (one paragraph), the statement that **ndsq adapts the index model of Google tensorstore** (link the design doc), the five message kinds with a one-line example each, and pointers to `spec/ndsq.md`, `schema/ndsq.schema.json`, `conformance/`, and the three implementation directories (noting Rust is implemented; Python/TypeScript are Plans 2–3).

- [ ] **Step 2: Full verification**

Run: `cargo test -p ndsq 2>&1 | tail -20`
