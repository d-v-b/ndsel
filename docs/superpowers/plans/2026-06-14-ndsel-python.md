# ndsel Python Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** An idiomatic Python peer implementation of ndsel that passes the same `/conformance/` corpus as the Rust reference.
...
**Tech Stack:** Python 3.13, `uv`, `pytest`, `jsonschema` (test-only), hatchling build backend. Reference: `spec/ndsel.md`, the Rust crate `rust/ndsel/`, and the frozen corpus `conformance/`.

---

## Scope of this plan

- **In:** the full Python package mirroring the Rust reference — values/bounds, domain canonicalization, three output-map kinds, transform canonicalization, all four shorthand desugarers, TypedDict message types, dataclass builders, `parse`/`normalize`, error codes, and the conformance runner. Fix the broken scaffolding (`src/ndsel/__init__.py` is a **directory**; stray `main.py`).
...
```
python/ndsel/pyproject.toml              project + dev deps (pytest, jsonschema) + hatchling
python/ndsel/src/ndsel/__init__.py        public API: parse, normalize, re-exports
python/ndsel/src/ndsel/errors.py          Reason (str Enum) + NdsqError
python/ndsel/src/ndsel/values.py          IndexValue alias, ImplicitValue dataclass, bound parse/emit
python/ndsel/src/ndsel/domain.py          Domain dataclass + canonicalize_domain
python/ndsel/src/ndsel/output.py          ConstantMap/SingleInputDimension/IndexArrayMap + canonicalize + to_json
python/ndsel/src/ndsel/transform.py       Transform dataclass + canonicalize_transform + identity + to_dict
python/ndsel/src/ndsel/messages.py        TypedDicts: Point/Box/Slice/Points/Transform message + Message union
python/ndsel/src/ndsel/builders.py        Point/Box/Slice/Points builder dataclasses (.to_message)
python/ndsel/src/ndsel/shorthand.py       desugar_point/box/slice/points
python/ndsel/tests/test_*.py             per-module unit tests (TDD)
python/ndsel/tests/test_conformance.py   corpus runner (schema-validate + normalize + compare)
```

Each module has one responsibility, mirroring the Rust crate's modules so the two implementations read as peers.

**Environment note for all tasks:** run every command from `/Users/d-v-b/dev/ndsel/python/ndsel`. Tests run via `uv run pytest`. The corpus and schema live at the repo root (`/Users/d-v-b/dev/ndsel/conformance`, `/Users/d-v-b/dev/ndsel/schema`), reachable from a test file as `Path(__file__).parents[3]`.
...
**Files:**
- Delete: `python/ndsel/main.py`, `python/ndsel/src/ndsel/__init__.py` (it is a stray **directory**)
- Modify: `python/ndsel/pyproject.toml`
- Create: `python/ndsel/src/ndsel/__init__.py` (empty for now), `python/ndsel/tests/__init__.py`

- [ ] **Step 1: Remove the broken scaffolding**

```bash
cd /Users/d-v-b/dev/ndsel/python/ndsel
rm -f main.py
rm -rf src/ndsel/__init__.py   # this is a directory, not a file
```

- [ ] **Step 2: Write pyproject.toml**

Replace `python/ndsel/pyproject.toml` with:
...
Create `python/ndsel/src/ndsel/__init__.py` containing a single line:

```python
"""ndsel — JSON-serializable n-dimensional spatial queries."""
```

Create an empty `python/ndsel/tests/__init__.py` (zero bytes).

- [ ] **Step 4: Verify the toolchain resolves**

Run: `uv run python -c "import ndsel; print('ok')"`
...
```bash
git add -A python/ndsel
git commit -m "chore(py): fix scaffolding, configure pyproject for ndsel package"
```
...
**Files:**
- Create: `python/ndsel/src/ndsel/errors.py`
- Test: `python/ndsel/tests/test_errors.py`

- [ ] **Step 1: Write the failing test**

Create `python/ndsel/tests/test_errors.py`:
...
Expected: FAIL — `ModuleNotFoundError: No module named 'ndsel.errors'`.
...
```bash
git add python/ndsel/src/ndsel/errors.py python/ndsel/tests/test_errors.py
...
**Files:**
- Create: `python/ndsel/src/ndsel/values.py`
- Test: `python/ndsel/tests/test_values.py`
...
Create `python/ndsel/tests/test_values.py`:
...
Expected: FAIL — `No module named 'ndsel.values'`.
...
```bash
git add python/ndsel/src/ndsel/values.py python/ndsel/tests/test_values.py
...
**Files:**
- Create: `python/ndsel/src/ndsel/domain.py`
- Test: `python/ndsel/tests/test_domain.py`
...
Create `python/ndsel/tests/test_domain.py`:
...
Expected: FAIL — `No module named 'ndsel.domain'`.
...
```bash
git add python/ndsel/src/ndsel/domain.py python/ndsel/tests/test_domain.py
...
**Files:**
- Create: `python/ndsel/src/ndsel/output.py`
- Test: `python/ndsel/tests/test_output.py`
...
Create `python/ndsel/tests/test_output.py`:
...
Expected: FAIL — `No module named 'ndsel.output'`.
...
```bash
git add python/ndsel/src/ndsel/output.py python/ndsel/tests/test_output.py
...
**Files:**
- Create: `python/ndsel/src/ndsel/transform.py`
- Test: `python/ndsel/tests/test_transform.py`

- [ ] **Step 1: Write the failing test**

Create `python/ndsel/tests/test_transform.py`:

```python
from ndsel.transform import Transform, canonicalize_transform
...
Expected: FAIL — `No module named 'ndsel.transform'`.
...
```bash
git add python/ndsel/src/ndsel/transform.py python/ndsel/tests/test_transform.py
...
**Files:**
- Create: `python/ndsel/src/ndsel/messages.py`, `python/ndsel/src/ndsel/builders.py`
- Test: `python/ndsel/tests/test_builders.py`

TypedDicts type the JSON message layer; builder dataclasses give ergonomic construction (`.to_message()` → the TypedDict).

- [ ] **Step 1: Write the failing test**

Create `python/ndsel/tests/test_builders.py`:
...
Expected: FAIL — `No module named 'ndsel.builders'`.

- [ ] **Step 3: Write `messages.py`**

Create `python/ndsel/src/ndsel/messages.py`:
...
Create `python/ndsel/src/ndsel/builders.py`:
...
```bash
git add python/ndsel/src/ndsel/messages.py python/ndsel/src/ndsel/builders.py python/ndsel/tests/test_builders.py
...
**Files:**
- Create: `python/ndsel/src/ndsel/shorthand.py`
- Modify: `python/ndsel/src/ndsel/__init__.py`
- Test: `python/ndsel/tests/test_shorthand_point_box.py`

- [ ] **Step 1: Write the failing test**

Create `python/ndsel/tests/test_shorthand_point_box.py`:
...
Expected: FAIL — `cannot import name 'normalize' from 'ndsel'`.
...
Replace `python/ndsel/src/ndsel/__init__.py` with:
...
```bash
git add python/ndsel/src/ndsel/shorthand.py python/ndsel/src/ndsel/__init__.py python/ndsel/tests/test_shorthand_point_box.py
...
**Files:**
- Modify: `python/ndsel/src/ndsel/shorthand.py`
- Test: `python/ndsel/tests/test_shorthand_slice.py`
...
Create `python/ndsel/tests/test_shorthand_slice.py`:
...
Replace the `desugar_slice` stub in `python/ndsel/src/ndsel/shorthand.py` with:
...
```bash
git add python/ndsel/src/ndsel/shorthand.py python/ndsel/tests/test_shorthand_slice.py
...
**Files:**
- Modify: `python/ndsel/src/ndsel/shorthand.py`
- Test: `python/ndsel/tests/test_shorthand_points.py`
...
Create `python/ndsel/tests/test_shorthand_points.py`:
...
Replace the `desugar_points` stub in `python/ndsel/src/ndsel/shorthand.py` with:
...
```bash
git add python/ndsel/src/ndsel/shorthand.py python/ndsel/tests/test_shorthand_points.py
...
**Files:**
- Test: `python/ndsel/tests/test_parse.py`

`parse`/`normalize` already implement this (Task 8). These lock the behavior in.

- [ ] **Step 1: Write the tests**

Create `python/ndsel/tests/test_parse.py`:
...
```bash
git add python/ndsel/tests/test_parse.py
...
**Files:**
- Create: `python/ndsel/tests/test_conformance.py`
...
SCHEMA_PATH = REPO_ROOT / "schema" / "ndsel.schema.json"
...
```bash
git add python/ndsel/tests/test_conformance.py
...
**Files:**
- Modify: `python/ndsel/README.md`

- [ ] **Step 1: Write the package README**

Replace `python/ndsel/README.md` with: one paragraph on what the package is (the Python peer implementation of ndsel, passing the shared conformance corpus); the two-layer model (TypedDict messages + dataclass builders/results); a short usage example:
...
and a note that it adapts tensorstore's index model (link `../../spec/ndsel.md`) and is validated against `../../conformance/`.
...
Run: `uv run python -c "from ndsel import normalize, Box; print(normalize(Box(shape=[3])).to_dict())"`
Expected: prints the canonical transform dict for a `[0,3)` box.

- [ ] **Step 3: Commit**

```bash
git add python/ndsel/README.md
