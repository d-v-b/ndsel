...
from ndsel.values import MaybeImplicitValue, parse_index_value
...
def test_bare_value_is_explicit():
    v = MaybeImplicitValue.from_json(7)
...
def test_bracketed_sentinel_is_implicit():
    v = MaybeImplicitValue.from_json(["-inf"])
...
def test_multi_element_bracket_is_invalid():
    with pytest.raises(NdselError) as e:
        MaybeImplicitValue.from_json([1, 2])
    assert e.value.reason is Reason.INVALID_JSON


def test_explicit_constructor():
    assert MaybeImplicitValue.explicit(0) == MaybeImplicitValue(value=0, implicit=False)
