...
Python peer implementation of **ndsq** — a JSON-serializable representation of n-dimensional array indexing ("spatial query"). It passes the same shared conformance corpus as the Rust reference.
...
```python
>>> from ndsq import normalize, Box
...
ndsq adapts [tensorstore's index model](../../spec/ndsq.md). All inputs are normalized to a canonical `Transform` that matches the spec.
